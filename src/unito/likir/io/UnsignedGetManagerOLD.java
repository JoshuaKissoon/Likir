...
public class UnsignedGetManagerOLD extends ObservableFutureTask<LinkedList<HashMap<String,Integer>>>
{
	private Node node;
	...
	private LinkedList<HashMap<String,Integer>> result;
	...
	public UnsignedGetManagerOLD(Node node, NodeId key, String type, String owner, boolean recent, int contentNumber)
	{
		this.node = node;
		this.type = type;
		this.key = key;
		...
		this.result = new LinkedList<HashMap<String,Integer>>();
	...
	public void run()
	{
		...
		HashMap<String,Integer> localEntries = node.getStorage().getCount(key, type, owner, recent);
		
		if (localEntries != null)
			result.add(localEntries);
		
		while (!(oldClosestNode == closestNode) && result.size() < contentNumber)
		{
			...
			try
			{
				...
				if (oldClosestNode == closestNode && result.size() < contentNumber)
				{
					doQueryRound(K,K);
					int oldSize = 0;
				    int size = markedList.size();
				    
					while(size < K && size != oldSize && result.size() < contentNumber)
					{
