...
public class UnsignedGetManager extends ObservableFutureTask<LinkedList<HashMap<String,Integer>>>
{
	private Node node;
	...
	private LinkedList<HashMap<String,Integer>> contentList;
	...
	public UnsignedGetManager(Node node, NodeId key, String type, String owner, boolean recent, int contentNumber)
	{
		this.node = node;
		this.type = type;
		this.key = key;
		...
		this.contentList = new LinkedList<HashMap<String,Integer>>();
	...
	public void run()
	{
		...
		HashMap<String,Integer> localEntries;
		
		localEntries = node.getStorage().getCount(key, type, owner, recent);
		
		if (localEntries != null)
			contentList.add(localEntries);
	...
	private void doQueryRound(int n, int range)
	{
		...
		for (Couple<Contact,Future<RPCMessage>> replyCouple : replies)
		{
			...
			try
			{
				...
				{
					contentList.add(reply.getCounters());
				}
