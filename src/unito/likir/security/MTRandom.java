...
public class MTRandom extends Random {
	...
	public final synchronized int getUniform(int a, int b)
	{
		double x = a + nextDouble() * (b-a);
		int result = (int)Math.floor(x);
		return result;
	}
	
	public final synchronized double getExpNeg(double mu)
	{
	    return ((-Math.log(nextDouble())) / mu);
	}
	
	public final synchronized double getPareto(double k, double x)
	{
		double u = nextDouble();
		return x / (Math.pow(u,1/k)); 
	}
	
	public final synchronized double getStdNormal()
	{
		double r = 0;
		for (int i=1; i<=12; i++)
		{
	...
	public final synchronized double getGaussian(double mean, double var)
	{
		return mean + nextDouble() * Math.sqrt(var);
	}
	
	public final synchronized double[] getZipfCumulativeFrequencies(int n, double exp)
	{
		double H = 0;
		...
		for (int k=1; k<=n; k++)
		{
			prob = (1/(double)(Math.pow(k,exp))) / H;
			cumulative += prob;
			distr[k-1] = cumulative;
		}
		return distr;
	}
	...
	public static void main(String... args)
	{
		...
		for (int i = 0; i<100; i++)
		{
			nextRunTime = Math.round(rand.getPareto(0.5, 1));
			System.out.println(nextRunTime);
		}*/
		/*int n = 100000;
		double[] probs = rand.getZipfCumulativeFrequencies(n, 1);
		for (int i = 0; i<n; i++)
		{
		...
		for (int i = 0; i<n; i++)
		{
			System.out.println(" > " + rand.selectRandom(probs));
		} */
		
		double x;
		for (int i = 0; i<100; i++)
		{
			x = rand.getStdNormal() + 10;
			System.out.println(" > " + x);
		}
	}
	
	public synchronized int selectRandom(double[] arr)
	{
		double p = nextDouble();
		return search(p,arr);
	}
	
	private int search(double val, double[] arr)
	{
